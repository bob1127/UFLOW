...
function isUpgradeCode(code) { return String(code || "").toLowerCase().startsWith("ufup-"); }
function isBirthdayCode(code) { return String(code || "").toLowerCase().startsWith("ufbd-"); }
function isReferralAmbCode(code) { return String(code || "").toLowerCase().startsWith("ufamb-"); }
function isReferralFriendCode(code) { return String(code || "").toLowerCase().startsWith("uffrd-"); }
function couponTitleByKindOrCode(c) {
  const code = String(c?.code || "");
  const kind = String(c?.kind || "").toLowerCase();
  const amount = Number(c?.amount) || 0;
  if (kind === "upgrade" || isUpgradeCode(code)) return `升等禮 - ${currency(amount)}`;
  if (kind === "birthday" || isBirthdayCode(code)) return `生日禮金 - ${currency(amount)}`;
  if (kind === "ref_ambassador_200" || isReferralAmbCode(code)) return `推薦折扣金 - ${currency(amount)}`;
  if (kind === "ref_friend_50" || isReferralFriendCode(code)) return `推薦折扣金 - ${currency(amount)}`;
...
function calcPricing(
  ...
  // 4. 扣除優惠券
  const safeCouponDiscount = Math.min(Math.max(Number(couponDiscount) || 0, 0), subtotalAfterMember);
...
// UI Components (維持原樣)
function CouponPicker({ title = "可用折價券", subtitle, coupons, applied, onApply, onClear, loading, emptyText = "目前沒有可用折價券" }) {
    const fmtValid = (c) => {
    ...
    <div className="bg-gradient-to-r from-[#fef9c3] to-[#fffbeb] border border-yellow-200 text-yellow-800 px-4 py-3 rounded-xl mb-6 flex items-start gap-3 shadow-sm">
      ...
      <div className="text-sm">
        <p className="font-bold">
          再消費 <span className="text-red-600 font-black">{currency(membership.nextNeedAmount)}</span> 
          {' '}即可升等為 <span className="text-black font-black">{membership.nextTierName}</span>！
        </p>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 grid lg:grid-cols-12 gap-12">
      <div className="lg:col-span-8">
        <h1 className="text-3xl font-bold tracking-tight mb-6">購物車 ({items.length})</h1>
        ...
        <div className="bg-gray-50 rounded-3xl p-8 sticky top-24 border border-gray-100">
          ...
          <div className="space-y-4 mb-6 text-sm">
            ...
            {pricing.memberDiscountAmount > 0 && (
               <div className="flex justify-between text-gray-500">
                ...
                <span className="font-black text-black">- {currency(pricing.memberDiscountAmount)}</span>
              </div>
            )}
            ...
            {pricing.couponDiscount > 0 && (
              <div className="flex justify-between text-gray-500">
                <span>折價券</span>
                <span className="font-black text-black">- {currency(pricing.couponDiscount)}</span>
              </div>
            )}
          ...
          <div className="border-t border-gray-200 pt-4 mb-8">
            ...
            {(pricing.memberDiscountAmount > 0 || pricing.couponDiscount > 0) ? (
              <p className="mt-2 text-[11px] text-gray-400">已省下：{currency(pricing.memberDiscountAmount + pricing.couponDiscount)}</p>
            ) : null}
          </div>
...
function CheckoutStep({
  ...
  const submit = async () => {
    ...
    const payload = {
        ...
        shipMethod,
        payMethod,
        coupon: appliedCoupon ? { code: appliedCoupon.code, amount: appliedCoupon.amount } : null,
        total: pricing.total,
        // ✅ 傳送會員折扣金額給後端 (若後端支援)
    ...
    <div className="max-w-6xl mx-auto px-4 py-12 flex flex-col lg:flex-row gap-12">
      <div className="flex-1 space-y-10">
        <section>
          ...
          <Input label="電子郵件" required type="email" value={contact.email} onChange={(e) => setContact({ ...contact, email: e.target.value })} error={errors.email} placeholder="您的電子信箱" />
        </section>
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-3 gap-3 mb-8">
            <button type="button" onClick={() => { setShipMethod("000"); setAddr({ ...addr, line1: "", storeId: "", storeName: "" }); }} className={`flex flex-col p-4 border-2 rounded-2xl text-left transition-all hover:border-black ${shipMethod === "000" ? "border-black bg-gray-50 shadow-inner" : "border-gray-100"}`}>
              <Truck className="mb-2 w-4 h-4 text-gray-400" />
              <span className="font-bold text-xs">宅配速送</span>
            </button>
            <button type="button" onClick={openEzShipMap} className={`flex flex-col p-4 border-2 rounded-2xl text-left transition-all hover:border-black ${shipMethod === "CVS" ? "border-black bg-gray-50 shadow-inner" : "border-gray-100"}`}>
              <MapPin className="mb-2 w-4 h-4 text-green-600" />
              <span className="font-bold text-xs">全家/萊爾富/OK</span>
            </button>
            <button type="button" onClick={openEcpay711Map} className={`flex flex-col p-4 border-2 rounded-2xl text-left transition-all hover:border-black ${shipMethod === "711" ? "border-black bg-gray-50 shadow-inner" : "border-gray-100"}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <Input label={shipMethod === "000" ? "配送地址" : "已選擇之門市"} required readOnly={shipMethod === "CVS" || shipMethod === "711"} value={addr.line1} onChange={(e) => setAddr({ ...addr, line1: e.target.value })} error={errors.line1} placeholder={shipMethod === "000" ? "請輸入完整街道路名與門牌" : "請點選上方按鈕選擇門市"} />
            </div>
            <div className="col-span-2">
        ...
        <section>
          ...
          <div className="p-6 border-2 border-black bg-gray-50 rounded-2xl flex items-center gap-4 shadow-inner">
            <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center border border-gray-100 shadow-sm"><CreditCard className="w-5 h-5" /></div>
            <div><p className="font-bold text-sm">綠界科技 ECPay 安全支付</p><p className="text-[11px] text-gray-500">支援信用卡、ATM、超商代碼</p></div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-10 border-t border-gray-100">
          <button type="button" onClick={onPrev} className="text-sm font-bold flex items-center gap-2 text-gray-400 hover:text-black transition group"><ChevronLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> 返回購物車</button>
          <button type="button" onClick={submit} disabled={isSubmitting} className="w-full sm:w-auto px-12 py-4 bg-black text-white rounded-2xl font-black text-lg hover:shadow-2xl hover:shadow-black/20 transition-all active:scale-95 disabled:opacity-50">{isSubmitting ? "建立訂單中..." : "確認付款並下單"}</button>
        </div>
      </div>

      <aside className="w-full lg:w-[380px]">
        <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm lg:sticky lg:top-24">
          <CouponPicker title="可用折價券" subtitle="升等禮 / 生日禮金" coupons={coupons} applied={appliedCoupon} onApply={onApplyCoupon} onClear={onClearCoupon} loading={couponLoading} />
          
          <h3 className="font-bold mb-6 flex items-center gap-2">訂單明細 <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded-full">{items.length}</span></h3>
          ...
          <div className="space-y-3 pt-6 border-t border-gray-50 text-xs">
            <div className="flex justify-between text-gray-400"><span>小計</span><span>{currency(pricing.subtotal)}</span></div>
            ...
            {pricing.couponDiscount > 0 ? (
              <div className="flex justify-between text-gray-400"><span>折扣</span><span className="font-black text-black">- {currency(pricing.couponDiscount)}</span></div>
            ) : null}
            <div className="flex justify-between text-gray-400"><span>運費</span><span>{pricing.shipping === 0 ? "免運" : currency(pricing.shipping)}</span></div>
            <div className="flex justify-between text-base font-black pt-3 border-t border-gray-100 mt-2"><span>總金額.</span><span>{currency(pricing.total)}</span></div>
          </div>
        </div>
      </aside>
    </div>
...
function CartContent() {
  ...
  const [pricing, setPricing] = useState({ subtotal: 0, memberDiscountAmount: 0, couponDiscount: 0, discountedSubtotal: 0, shipping: 0, total: 0 });
  const [contact, setContact] = useState({ email: "" });
  const [addr, setAddr] = useState({ firstName: "", lastName: "", line1: "", phone: "", storeId: "", storeName: "", storeAddr: "" });
  ...
  useEffect(() => {
    ...
    if (sId && sName) {
      ...
      setAddr(prev => ({ ...prev, line1: `${sName} (${sId})`, storeId: sId, storeName: sName, storeAddr: sAddr || "" }));
      setStep(2);
    } else {
  ...
  useEffect(() => {
    const loadAllCoupons = async () => {
      ...
      try {
        const res = await fetch("/api/account/coupons/available", { cache: "no-store", credentials: "include" });
        const data = await res.json();
        ...
        arr.forEach(c => {
          ...
          const kind = String(c.kind || "").toLowerCase();
          const item = { code: c.code, amount: Number(c.amount) || 0, kind: c.kind || "", title: couponTitleByKindOrCode(c) };
  ...
  useEffect(() => {
    ...
    // ✅ 將 discountRate 傳入計算
    setPricing(calcPricing(items, { shippingBase: 0, freeShipThreshold: 1800 }, discount, discountRate));
  ...
  if (!itemsLoaded) return <div className="h-[60vh] flex items-center justify-center"><div className="w-10 h-10 border-4 border-gray-100 border-t-black rounded-full animate-spin" /></div>;
    ...
    <div className="min-h-screen bg-white">
      <header className="border-b sticky top-0 bg-white/90 backdrop-blur-xl z-50">
        <div className="max-w-6xl mx-auto h-20 flex items-center justify-between px-6">
          <a href="/"><img src="/images/logo/logo-y.png" alt="Uflow" className="h-7" /></a>
...
export default function CartPage() {
  return (
    <Suspense fallback={<div className="h-screen flex items-center justify-center font-black tracking-widest text-gray-200">UFLO</div>}>
      <CartContent />
    </Suspense>
  );
}